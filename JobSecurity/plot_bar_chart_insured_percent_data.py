...
def create_x_and_counts(df=None, cities=None):
    ...
    cnts = [df_selected.loc[df_selected.index == city, 'uninsured_percent'].values[0]
...
#########################
df = pd.read_csv('data/Job_Security/uninsured_percent.csv')
df['city'] = df['cities']

#########################
cities = []
cities.append(df.loc[df.uninsured_percent == np.max(df.uninsured_percent), 'city'].values[0])
cities.append(df.loc[df.uninsured_percent == np.median(df.uninsured_percent[1:]), 'city'].values[0])
cities.append(df.loc[df.uninsured_percent == np.min(df.uninsured_percent), 'city'].values[0])
...
#########################
columns_to_select = ['uninsured_percent']
...
p = figure(x_range=FactorRange(*source.data['x']),
           ...
           title="uninsured percent comparison",
...
# bokeh serve --show JobSecurity/plot_bar_chart_insured_percent_data.py
# python -m notebook
