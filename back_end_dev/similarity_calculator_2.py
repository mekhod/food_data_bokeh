...
for m in list(df.metro_normed):
    dict_temp = {}
    metro_data = df.loc[df.metro_normed == m, :]

    ##
    list_vote = []
    for voting_likelihood in [0, 1]:
        list_vote.append(round(voting_likelihood - metro_data['voters_ratio'].values.squeeze(), 2))
    dict_temp['vtd'] = list_vote
    ...
    for fruit_per_day in np.linspace(0.5, 2.0, 4):
        ...
        if food_first_pc_std > 1:
            food_first_pc_std = 1
        list_fruit.append(round(food_first_pc_std - metro_data['food_first_pc'].values.squeeze(), 2))
    dict_temp['fd'] = list_fruit
    ...
    for education in ['LESS THAN 1ST GRADE',
        ...
        list_education.append(round(education_rank_std -
                                    metro_data['education_weight'].values.squeeze(), 2))
    dict_temp['edd'] = list_education

    ##
    list_park_access = []
    for park_access in [0, 1]:
        list_park_access.append(round(park_access -
                                      metro_data['park_access_ratio'].values.squeeze(), 2))
    dict_temp['pad'] = list_park_access

    ##
    list_insured = []
    for insured in [0, 1]:
        uninsured = 1 - insured
        list_insured.append(round(uninsured -
                                  metro_data['uninsured_ratio'].values.squeeze(), 2))

    ##
    dict_all_metro_sim[m] = dict_temp
