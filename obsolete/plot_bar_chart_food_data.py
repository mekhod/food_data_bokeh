...
def create_x_and_counts(df=None, cities=None, list_of_determinants=['_FLSHOT6', '_PNEUMO2']):
    ...
    for determinant in list(df_selected.columns):
        if len(np.unique(list(df_selected.loc[:, determinant]))) < 30:
            print("less than 30")
            x_ext = [(str(determinant), str(cat), str(city))
                     for city in list(np.unique(list(df_selected.index)))
                     for cat in np.unique(list(df_selected.loc[:, determinant]))]
            x.extend(x_ext)

            cnts_ext = [df_selected.loc[(df_selected.index == city) *
                                        (df_selected[[determinant]] == cat).values.squeeze(),
                                        determinant].shape[0] /
                        df_selected.loc[df_selected.index == city, determinant].shape[0] * 100
                        for city in list(np.unique(list(df_selected.index)))
                        for cat in list(np.unique(list(df_selected.loc[:, determinant])))]
            cnts.extend(cnts_ext)
        else:
            print("greater than 30")
            x_ext = [(str(determinant), 'avg_' + str(determinant), str(city))
                     for city in list(np.unique(list(df_selected.index)))]
            x.extend(x_ext)

            cnts_ext = [np.nanmean(df_selected.loc[(df_selected.index == city),
                                                   determinant].values)
                        for city in list(np.unique(list(df_selected.index)))]
            cnts.extend(cnts_ext)
    return x, cnts
    ...
    ##################################################
    # x = [(str(determinant), str(cat), str(city))
    #      for city in list(np.unique(list(df_selected.index)))
    #      for determinant in list(df_selected.columns)
    #      for cat in np.unique(list(df_selected.loc[:, determinant]))]
    #
...
def update_plot(attrname, old, new):
    ...
    # clrs = palette[0: len(cities)] #*(int(len(x_new) / len(cities))))
...
# bokeh serve --show plot_bar_chart_food_data.py
